...
可以通过 --prefix 参数指定视频名的前缀

python video_rename_prefix.py --videos_path ./videos --video_start 0 --prefix damper_
...
# 添加一个新的参数，用于指定视频名的前缀
parser.add_argument('--prefix', type=str, default='', help='Prefix to add to the video names')
...
def rename_videos_in_folder(folder_path, start_value, prefix):
    # 确保提供的路径存在且是文件夹
...
        # 构建新的文件名，添加前缀
        new_file_name = f"{prefix}{i:04d}{os.path.splitext(video_file)[1]}"
...
rename_videos_in_folder(videos_path, video_start, prefix)
