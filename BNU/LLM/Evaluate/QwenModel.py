...
def argparser():
    ...
    parser.add_argument('--dataset', default='/root/autodl-tmp/Data/experimental_Bridge', type=str)
    parser.add_argument('--output', default='./output/result.txt', type=str)
    parser.add_argument('--model_path', default="/root/autodl-tmp/Qwen/Qwen/Qwen2-VL-7B-Instruct-fine-tuning", type=str)
...
def process():
    ...
    for subdir in tqdm(subdirs, desc="Processing subdirectories"):
        ...
        for _, row in data.iterrows():
            ...
            # 保存结果
            with open(configs.output, 'a', encoding='utf-8') as f:
                f.write(f"image_path: {image_path}\n")
                f.write(f"predicted_labels: {predicted_labels}\n")
                f.write(f"true_labels: {true_labels}\n")
