# 这一段代码是将via转化为yolo格式 分train与val是采用视频为单位区分，而非图片
# python via2yolo5.py --via_Dataset  ./via-2023-12-31/ --yolo_Dataset ./yolo_behavior_Dataset-2023-12-31 --tain_r 0.8
# python via2yolo5.py --via_Dataset  ./frams_explore_moon01/ --yolo_Dataset ./yolo_frams_explore_moon01 --tain_r 0.8
...
for root, dirs, files in os.walk(via_Dataset, topdown=False):
    for name in files:
        ...
        if '.json' in name:
            ...
            # 假设json文件名为：8030.json，那么就把30存入train_video或者val_video
            cur_video = int(name.split('.')[0])%1000

            if cur_video in train_video:
                train_val = True
            else:
                if random.random() <= tain_r and cur_video not in val_video:
                    train_val = True
                    train_video.append(cur_video)
                else:
                    train_val = False
                    val_video.append(cur_video)
            
            file_json = open(file_dir, 'r', encoding='utf-8')
